...
if [ "$#" -lt 7 ] || [ "$#" -gt 8 ]
then
	echo "usage: sitl_run.sh rc_script rcS_dir debugger program model src_path build_path home(optional)"
	echo ""
   echo "    home format:<lat>,<lon>,<alt> "
   echo "         (no spaces)" 
	exit 1
fi
...
if [ "$home" == "" ]
then
   jmavsim_home=37.464184,-122.118011,0
else
   jmavsim_home=$home
fi
...
then
   ...
   echo "JMAVSIM_COMMAND: $src_path/Tools/jmavsim_run_loc.sh -h $jmavsim_home &"
	$src_path/Tools/jmavsim_run_loc.sh -h $jmavsim_home &
	SIM_PID=`echo $!`
	cd ../..
